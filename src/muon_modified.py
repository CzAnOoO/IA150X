...
Muon optimizer with 4D support (Custom Implementation)

This implementation is a functional fork of the official 'torch.optim.Muon', extended to support 4D conv layers in ResNet.
Original source: https://github.com/pytorch/pytorch/blob/main/torch/optim/_muon.py

Modifications (additions/deletions) are annotated throughout the code.
...
class Muon(Optimizer):
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            # ---------------------------
            if p.ndim == 4:
                params_with_grad.append(p.view(p.size(0), -1))
                grads.append(p.grad.view(p.size(0), -1))
            else:
                params_with_grad.append(p)
                grads.append(p.grad)
            """ ==================================== """
            ...
            # ---------------------------
            if p.ndim == 4:
                muon_momentum_bufs.append(state["momentum_buffer"].view(p.size(0), -1))
            else:
                muon_momentum_bufs.append(state["momentum_buffer"])
            """ ==================================== """
